...
module.exports = function updated(data, options) {
  ...
  if (data === undefined) {
    sails.log.info('Ran custom response: res.updated()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.updated()` called with an Error:', result);
