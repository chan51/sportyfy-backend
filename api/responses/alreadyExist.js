...
module.exports = function alreadyExist(data, options) {
  
  // Get access to `req` and `res`
  var req = this.req;
  var res = this.res;
  var result = { data, status: false, ...options };
  ...
  if (data === undefined) {
    sails.log.info('Ran custom response: res.alreadyExist()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.alreadyExist()` called with an Error:', result);
